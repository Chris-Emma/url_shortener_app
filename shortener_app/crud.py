...
def create_db_url(db: Session, url: schemas.URLBase) -> models:
    ...
    Returns:
    - URL: Newly created URL entry in the database
    """
...
def get_db_url_by_key(db: Session, url_key:str) -> models.URL:
    """
    Retrieve a URL entry from the database based on the given key.

    Parameters:
    - db (Session): SQLAlchemy database session
    - url_key (str): Key associated with the URL entry to be retrieved

    Returns:
    - URL: URL entry with the given key, or None if not found or inactive
    """
    return (
        db.query(models.URL)
